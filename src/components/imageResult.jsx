...
function ImageResult() {
  ...
  React.useEffect(() => {
    if (location.state) { // check if state exists
      ...
      {result ? (
        <div className="flex flex-col justify-center items-center gap-4">
          ...
          <div className="flex gap-32 justify-evenly items-center bg-[#252525] border border-[#ffffff10] rounded-xl px-12 py-2">
            <div className="text-xl font-medium">{vidname}</div>
            {result.score < 0.5 ? (
            ...
            <p className="flex gap-2 justify-center items-center text-md font-medium">
              <div className="h-2 w-2 bg-green-600 rounded-full"></div> Stored on Blockchain
            </p>
          </div>
          <button
        ...
        <div>No image result available.</div>
      )}
    </div>
  );
}
